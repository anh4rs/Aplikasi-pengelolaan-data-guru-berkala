...
<body>
    ...
    <div class="container">
        <div class="isi">
        <h2 style="text-align:center;text-transform: uppercase;">DATA  GURU PADA DIKLAT {{$diklat->nama}}</h2>
                <table  class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($diklat->diklat_guru as $p)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $p->guru->NIP }}</td>
                                <td>{{ $p->guru->nama }}</td>
                                <td>{{ $p->guru->sekolah->NPSN }}</td>
                                <td>{{ $p->guru->telepon }}</td>
                                <td>{{ $p->waktu }}</td>
                            </tr>
                            @endforeach
                        </tfoot>
